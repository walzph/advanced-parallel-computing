...
void transposeMatrix(float* a, float* aT, int m, int n)
{
	for(auto i = 0; i < m; ++i)
	{
		for(auto j = 0; j < n; ++j) { aT[j * m + i] = a[i * n + j]; }
	}
}
...
void transposeMatrix(float* a, uint16_t* aT, int m, int n, int bit_a)
{
	...
	for(auto i = 0; i < m; ++i)
	{
		for(auto j = 0; j < n; ++j) { aT[j * m + i] = (uint16_t) round(a[i * n + j] * fp_factor); }
	}
}
#endif
...
int ReLU(float* InputTensor, input_t* QuantTensor, uint16_t* indices, int x, int y, float threshold, int bit_a)
{
	...
	for(int i = 0; i < y; ++i)
	{
		...
		for(int j = 0; j < x; ++j)
		{
			...
			// InputTensor[i] = 2*InputTensor[i]-1;
			// std::cout << "Activation=" << InputTensor[i] << std::endl;
			if(InputTensor[i * x + j] == 0) { zero_count++; }
			// else
			{
				// QuantTensor[i*x+col_count] = round(InputTensor[i*x+j]*n);
...
void BatchnormalizationRMO(float* InputTensor, int x, int y, float* beta, float* gamma, float* mean, float* variance)
{
	...
	for(int i = 0; i < x; ++i)
	{
		for(int j = 0; j < y; ++j)
		{
			...
			InputTensor[i * y + j] =
			    ((InputTensor[i * y + j] - mean[j]) * gamma[j]) / sqrt(variance[j] + 1e-4) + beta[j];
		}
	}
}
...
void BatchnormalizationCMO(float* InputTensor, int x, int y, float* beta, float* gamma, float* mean, float* variance)
{
	...
	for(int i = 0; i < y; ++i)
	{
		for(int j = 0; j < x; ++j)
		{
			// TODO: gamma/sqrt(variance+epsilon) can be pre-computed
			InputTensor[i * x + j] =
			    ((InputTensor[i * x + j] - mean[i]) * gamma[i]) / sqrt(variance[i] + 1e-4) + beta[i];
		}
	}
}
...
{
	for(int i = 0; i < x; ++i) { zeta[i] = gamma[i] / sqrt(variance[i] + 1e-4); }
}
...
int initialize_input(float* InputTensor, mnist::MNIST_dataset<std::vector, std::vector<float>, uint8_t>& dataset,
                     int batch_size, int batch_id)
{
	...
	for(int frame_id = 0; frame_id < batch_size; frame_id++)
	{
		...
		{
			InputTensor[frame_id * frame_size + pixel_id] =
			    (float) dataset.test_images.at(batch_id * batch_size + frame_id).at(pixel_id);
			if(InputTensor[frame_id * frame_size + pixel_id] == 0) zero_count++;
		}
	}
#ifdef PRINT_STATS
	double sparsity = (double) (zero_count / (batch_size * frame_size)) * 100;
	std::cout << "[STATS] Found " << sparsity << "% sparsity in input batch, " << zero_count << "/"
	          << batch_size * frame_size << std::endl;
#endif
	return (int) zero_count;
}

void initialize_labels(int* labels, mnist::MNIST_dataset<std::vector, std::vector<float>, uint8_t>& dataset,
                       int batch_size, int batch_id)
{
	for(int frame_id = 0; frame_id < batch_size; frame_id++)
	{
...
int main(int argc, char* argv[])
{
	...
	// cnpy::npz_t parameters_npz = cnpy::npz_load("trained_models/mnist-float32-nn.npz");
	...
	unique_ptr<sparse_list_tuple[]> sparse_lists_0 =
	    createSparseList<NUM_NEURONS, FRAME_SIZE>(WeightTensor0T, Wp0, Wn0);
#endif
	free(WeightTensor0T);
	...
	unique_ptr<sparse_list_tuple[]> sparse_lists_1 =
	    createSparseList<NUM_NEURONS, NUM_NEURONS>(WeightTensor1T, Wp1, Wn1);
#endif
	free(WeightTensor1T);
	...
	unique_ptr<sparse_list_tuple[]> sparse_lists_2 =
	    createSparseList<NUM_NEURONS, NUM_NEURONS>(WeightTensor2T, Wp2, Wn2);
#endif
	free(WeightTensor1T);
	...
	for(int batch_id = 0; batch_id < size_test_set / BATCH_SIZE; ++batch_id)
	{
		...
		unique_ptr<float[]> out_tensor_0 =
		    sparseMatrixMultiply<NUM_NEURONS, BATCH_SIZE>(Input0T, sparse_lists_0.get(), Wp0, Wn0);
#endif
...
#else
		BatchnormalizationCMO(out_tensor_0.get(), BATCH_SIZE, NUM_NEURONS, beta0, gamma0, mean0, variance0);
#endif
		...
		// SAFcBnReLUAVX2( OutputTensor0Tint8, indices, SparseList1, OutputTensor1T, Wp1, Wn1, BATCH_SIZE, NUM_NEURONS,
		// NUM_NEURONS, 8 );

#ifndef __NEW_SPARSE__
		SparseDotProductAVX2(OutputTensor0T, SparseList1, OutputTensor1T, Wp1, Wn1, BATCH_SIZE, NUM_NEURONS,
		                     NUM_NEURONS);
#else
		unique_ptr<float[]> out_tensor_1 =
		    sparseMatrixMultiply<NUM_NEURONS, BATCH_SIZE>(out_tensor_0.get(), sparse_lists_1.get(), Wp1, Wn1);
#endif

		// mat_cmp(OutputTensor1T, out_tensor_1.get());

		// SASparseDotProduct( SAOutputTensor0T, indices, SparseList1, OutputTensor1T, Wp1, Wn1, BATCH_SIZE,
		// NUM_NEURONS, NUM_NEURONS ); transposeMatrix( OutputTensor1T, OutputTensor1, NUM_NEURONS, BATCH_SIZE );
		// BatchnormalizationCMOZeta( OutputTensor1T, BATCH_SIZE, NUM_NEURONS, beta1, mean1, zeta1 );

#ifndef __NEW_SPARSE__
		BatchnormalizationCMO(OutputTensor1T, BATCH_SIZE, NUM_NEURONS, beta0, gamma0, mean0, variance0);
#else
		BatchnormalizationCMO(out_tensor_1.get(), BATCH_SIZE, NUM_NEURONS, beta0, gamma0, mean0, variance0);
#endif

#ifndef __NEW_SPARSE__
		zero_count +=
		    (unsigned long long int) ReLU(OutputTensor1T, OutputTensor1Tint8, BATCH_SIZE, NUM_NEURONS, 0.0 /*0.32*/, 8);
#else
		zero_count +=
		    (unsigned long long int) ReLU(out_tensor_1.get(), OutputTensor1Tint8, BATCH_SIZE, NUM_NEURONS, 0.0, 8);
#endif
		activation_count += (unsigned long long int) BATCH_SIZE * NUM_NEURONS;
...
#ifndef __NEW_SPARSE__
		SparseDotProductAVX2(OutputTensor1T, SparseList2, OutputTensor2T, Wp2, Wn2, BATCH_SIZE, NUM_NEURONS,
		                     NUM_NEURONS);
#else
		unique_ptr<float[]> out_tensor_2 =
		    sparseMatrixMultiply<NUM_NEURONS, BATCH_SIZE>(out_tensor_1.get(), sparse_lists_2.get(), Wp2, Wn2);
#endif

		// mat_cmp(OutputTensor2T, out_tensor_2.get());

		// FcBnReLUAVX2( OutputTensor1Tint8, SparseList2, OutputTensor2T, Wp2, Wn2, BATCH_SIZE, NUM_NEURONS,
		// NUM_NEURONS, 8 ); transposeMatrix( OutputTensor2T, OutputTensor2, NUM_NEURONS, BATCH_SIZE );
		// BatchnormalizationCMOZeta( OutputTensor2T, BATCH_SIZE, NUM_NEURONS, beta2, mean2, zeta2 );

#ifndef __NEW_SPARSE__
		BatchnormalizationCMO(OutputTensor2T, BATCH_SIZE, NUM_NEURONS, beta0, gamma0, mean0, variance0);
#else
		BatchnormalizationCMO(out_tensor_2.get(), BATCH_SIZE, NUM_NEURONS, beta0, gamma0, mean0, variance0);
#endif
	...
	// std::cout << "[RESULT] Sparsity       = " << sparsity << "%, " << zero_count << "/" << activation_count <<
	// std::endl;
